...
def test_extract_gist_id():
    assert (
        extract_gist_id(
            "https://gist.github.com/jacobtomlinson/" "c9852fa17d3463acc14dca1217d911f6"
        )
...
def test_get_config_option():
    ...
    result = get_config_option(
        ["repo", "path", "gist"], module_config, True, "opsdroid.connectors.websocket",
    )

    assert result == (True, "repo", "test")


def test_get_config_no_option():
    module_config = {
        "bot-name": "mybot",
        "max-connections": 10,
    ...
    result = get_config_option(["repo", "path", "gist"], module_config, True, "module")

    assert result == ("module", "module", "module")


def test_get_config_option_exception():
    module_config = None
