...
async def _load_model(config):
    ...
    async with aiohttp.ClientSession(trust_env=True) as session:
        ...
        if "token" in config:
            url += "?token={}".format(config["token"])
        try:
            resp = await session.put(url, data=json.dumps(data), headers=headers, timeout=300)
        except aiohttp.client_exceptions.ClientConnectorError:
            _LOGGER.error(_("Unable to connect to Rasa NLU."))
            return None
        if resp.status == 204:
...
async def _is_model_loaded(config):
    ...
    async with aiohttp.ClientSession(trust_env=True) as session:
        ...
        if "token" in config:
            url += "?token={}".format(config["token"])
        try:
            resp = await session.get(await _build_status_url(config), timeout=300)
        except aiohttp.client_exceptions.ClientConnectorError:
            _LOGGER.error(_("Unable to connect to Rasa NLU."))
            return None
        if resp.status == 200:
...
async def train_rasanlu(config, skills):
    ...
    async with aiohttp.ClientSession(trust_env=True) as session:
        ...
        try:
            ...
            resp = await session.post(url, data=intents, headers=headers, timeout=300000000)
        except aiohttp.client_exceptions.ClientConnectorError:
