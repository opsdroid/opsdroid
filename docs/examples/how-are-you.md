...
## Building the Skill
We are now ready to start writing the 'how are you' skill. We will be using the opsdroid regex matcher to write our function. The first thing to do inside our `__init__.py` will be import the `Skill` class from opsdroid to inherit from it.
...
#### Decorators
A matcher is meant to be used as a function decorator. The decorator allows the opsdroid to understand the function, so we need to use this decorator and use the regex that we wish the opsdroid to react to.
...
Let's write our function so the opsdroid knows what to trigger when a user writes the words `how are you?`
...
Opsdroid is built with asyncio. That means every function that you wish the opsdroid to react to, should be an asynchronous function.

_Note: that every function will take these two parameters: `self` and `message`._

#### Opsdroid Answer
So far, our opsdroid can match something that a user says and can trigger a skill. But nothing will happen yet. Let's make the opsdroid answer the text with a message of its own.
...
Our skills are done and opsdroid will be able to respond like on the [Opsdroid main page](https://opsdroid.github.io). The final thing left to do is to add the skill to the opsdroid configuration file.
